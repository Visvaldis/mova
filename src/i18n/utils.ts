...
// import.meta.env.BASE_URL is the configured `base` ("/mova/"). Normalize once.
const BASE = (import.meta.env.BASE_URL ?? '/').replace(/\/$/, '');

/** Prefix an absolute app path with the deploy base ("/mova"). Use for EVERY internal link. */
export function withBase(path = '/'): string {
  const rel = path.startsWith('/') ? path : `/${path}`;
  return `${BASE}${rel}` || '/';
}

/** Build a localized, base-prefixed, trailing-slash path: localizedPath('uk', 'origins') -> /mova/uk/origins/ */
...
/** Content collection ids are "<lang>/<slug>" (from the glob loader). Helpers to bridge id <-> slug. */
